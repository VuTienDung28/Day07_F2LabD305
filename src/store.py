...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        record_id = f"{doc.id}::{self._instance_id}::{self._next_index}"
        self._next_index += 1
        return {
            "id": record_id,
            "content": doc.content,
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        if self._use_chroma and self._collection is not None:
            try:
                ...
                # The in-memory copy remains the canonical fallback if an
                # optional Chroma installation is unavailable or incompatible.
                self._use_chroma = False
                self._collection = None
                self._client = None
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        self._store = [record for record in self._store if record["id"] not in removed_ids]

        if self._use_chroma and self._collection is not None:
            try:
                self._collection.delete(ids=list(removed_ids))
            except Exception:
                self._use_chroma = False
                self._collection = None
                self._client = None
