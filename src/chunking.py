...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Keep sentence-ending punctuation in the preceding sentence. The
        # expression covers the separators documented by the exercise while
        # also accepting repeated whitespace and Windows line endings.
        sentences = [
            sentence.strip()
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # No useful separator remains, so use a deterministic hard split.
        ...
        for index, part in enumerate(raw_parts):
            ...
            # Preserve the separator where possible so sentence/paragraph
            # punctuation is not silently removed from the stored content.
        ...
        # Merge adjacent small pieces to avoid producing one tiny chunk per
        # word or sentence while still respecting chunk_size.
        chunks: list[str] = []
        current_chunk = ""
        for piece in pieces:
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        for name, chunker in strategies.items():
            ...
            count = len(chunks)
            average_length = sum(len(chunk) for chunk in chunks) / count if count else 0.0
            comparison[name] = {
                "count": count,
